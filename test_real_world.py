...
def _print_scorecard(r):
    ...
    caution_count = len(r.get('calibration_cautions', [])) + len(r.get('gaming_risk_notes', [])) + len(r.get('evidence_cautions', []))
...
def _print_summary(results):
    ...
    if citable:
        ...
        print(f"       NIST AI 100-1, NHS DTAC) are not LAIF-native -- verified against")
        print(f"       hash-pinned authoritative sources; citable within excerpt scope.")
