...
Write the assessment to `reports/`. A machine-generated assessment belongs in the deterministic pipeline (`test_real_world.py`) so that CI regenerates and checks it; an authored narrative analysis is committed with a row in `reports/README.md` recording its production date and model version. Commit to `main`. No assessment is authoritative until on `main`, and a narrative analysis is authoritative only as reasoning at its stated date — the reproducibility guarantees in this document apply to the generated artifacts.
...
The reproducible assessment for the current corpus is
`reports/laif_real_world_assessment.md`, with `reports/laif_executive_summary.md`
and `reports/laif_assessment_data.json`. All three are regenerated by
`python3 test_real_world.py`, are byte-identical across runs, carry the corpus
fingerprint that identifies exactly which texts produced them, and are checked
against their committed copies by CI.

`reports/laif_full_assessment.md` is an authored narrative analysis from 5 May
2026 under Refined Model v1.1. It is retained as the assessment reference cited
by the verified corpus manifest, but it is not regenerated, not fingerprinted,
and does not reflect the current engine — the reproducibility guarantees in this
document do not apply to it. See `reports/README.md`.
