...
If a command cannot be run, state which command was skipped and why. Do not describe skipped checks as passing. Governance scripts emit stable `GOVERNANCE_RESULT status=PASS|WARN|ERROR` lines; `WARN` from semantic-boundary checks is advisory and does not by itself indicate semantic drift.

## Pull Request Expectations

Use `.github/pull_request_template.md` when opening a pull request. Every pull request should describe:
