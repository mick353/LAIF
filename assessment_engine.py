...
def generate_markdown_report(assessments, report_date="May 2026"):
    ...
    for idx, r in enumerate(assessments, 1):
        ...
        if es:
            rendered_verdict = es.get("verdict", "").replace("This document fails formal LAIF v1.2 compliance.", "This document does not pass the formal LAIF-native certification gate.").replace("Missing any single construct = FAIL", "Missing any single construct prevents LAIF-native certification")
            if rendered_verdict:
                p()
